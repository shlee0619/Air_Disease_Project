...
def plot_seasonal_patterns(data):
    ...
    sns.boxplot(x='season_name', y='pm10', data=data, palette='Blues')
    ...
    plt.figure(figsize=(12, 6))
    sns.boxplot(x='season_name', y='pm25', data=data, palette='Reds')
    ...
    for i, col in enumerate(medicine_cols, 1):
        plt.subplot(2, 2, i)
        sns.boxplot(x='season_name', y=col, data=data, palette='Greens')
    ...
    for i, col in enumerate(disease_cols, 1):
        plt.subplot(3, 2, i)
        sns.boxplot(x='season_name', y=col, data=data, palette='Oranges')
        plt.title(f'{col.replace("_", " ").title()} by Season')
        plt.xlabel('Season')
        plt.ylabel('Search Volume')
        plt.grid(True, alpha=0.3)
...
def analyze_lag_effects(data):
    ...
    # 데이터 준비
    heatmap_data = lag_corr_results.unstack().unstack(level=0)
    
    # PM10 시차 효과 히트맵
    plt.subplot(1, 2, 1)
    sns.heatmap(heatmap_data['pm10'], annot=True, fmt='.2f', cmap='coolwarm', center=0,
                linewidths=0.5, annot_kws={"size": 8})
    plt.title('Lag Correlation: PM10 vs Health Variables')
    plt.xlabel('Lag (Days)')
    plt.ylabel('Health Variables')
    
    # PM2.5 시차 효과 히트맵
    plt.subplot(1, 2, 2)
    sns.heatmap(heatmap_data['pm25'], annot=True, fmt='.2f', cmap='coolwarm', center=0,
                linewidths=0.5, annot_kws={"size": 8})
