...
def analyze_simple_lag_effects(data):
    """간소화된 시차 효과 분석"""
    print("\n간소화된 시차 효과 분석 중...")
    ...
    dust_vars = ['pm10', 'pm25']
    health_vars = [
        'cold_medicine', 'rhinitis_medicine', 'allergy_medicine',
    ...
    lag_corr_df = pd.DataFrame(columns=['dust_var', 'health_var', 'lag', 'correlation'])
    ...
    for dust_var in dust_vars:
        for health_var in health_vars:
            for lag in lags:
                ...
                # 결과 저장
                lag_corr_df = lag_corr_df._append({
                    'dust_var': dust_var,
    ...
    # 결과 저장
    lag_corr_df.to_csv(os.path.join(results_dir, 'lag_correlation.csv'), index=False)
    ...
    for health_var in key_health_vars:
        ...
        for dust_var in dust_vars:
            # 해당 변수 쌍의 시차 상관관계 추출
            var_data = lag_corr_df[(lag_corr_df['dust_var'] == dust_var) & 
                                   (lag_corr_df['health_var'] == health_var)]
            
            plt.plot(var_data['lag'], var_data['correlation'], marker='o', label=dust_var)
...
def exploratory_data_analysis(data):
    ...
    # 간소화된 시차 효과 분석
    analyze_simple_lag_effects(data)
